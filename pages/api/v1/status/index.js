...
async function status(request, response) {
  ...
  const databaseVersionResulte = await database.query("SHOW server_version;")
  const databaseVersionValue = databaseVersionResulte.rows[0].server_version
    ...
    .json({
      ...
      dependecies: {
        database: {
          ...
          opened_conecctions: databaseOpenConnectionsValue
        }
      }
    });
}
